...
security bugs to use broadly the same procedure as [other
...
ability willingness to fill out the relevant details. Often they go
much further than the template by including a long-form root-cause
...
reading it you can work out what is at issue is if you mess with a
...
through walls of plausible text from a seemingly never end of stream
of incoming reports. 

It's with some sense of irony that we've been experimenting with using
LLMs to do the initial triage of bugs so we can better distribute the
load by routing bugs to the appropriate developer. But still
eventually a human has to asses and decide what to do about it.
...
Nearly half of the currently open confidential issues are in under
maintained areas of the code base. This means although they have
triaged they are unlikely to be looked at by anyone. It's also not
particularly surprising that if you go looking around in dusty areas
of the code base you are likely to find warts that have been removed
