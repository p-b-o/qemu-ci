...
/*
 * The information block is always located at address 0x0.
 * Address and size are therefore replaced by two identifiers.
                              ...
                              unsigned size)
{
    ...
    case EEPROM_CMD_OFFSET:
        ...
        if (val & EEPROM_CMD_READ_MASK) {
            ...
            stw_le_p((s->eeprom_storage + eeprom_adr), buf);
            sync_eeprom(s);
        }
        break;
    default:
...
static uint64_t beckhoff_ccat_dma_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    case DMA_TRANSFER_OFFSET:
        /*
         * Real hardware keeps the 0x1 bit set while the DMA transfer is
         * running. Complete the emulated transfer only after software has
         * observed that intermediate state, since Beckhoff drivers expect the DMA
         * transfer not to finish immediately.
         */
                           ...
                           unsigned size)
{
    ...
    case DMA_TRANSFER_OFFSET:
        ...
        dma_addr = ldl_le_p(&s->mem[DMA_HOST_ADR_OFFSET]);
        if (s->mem[DMA_DIRECTION_OFFSET] & DMA_DIRECTION_MASK) {
             ...
             * section 20.2.
             */
            dma_memory_write(&address_space_memory, dma_addr + 8,
                                mem_buf, len * 8, MEMTXATTRS_UNSPECIFIED);
        }
        break;
    }
...
static void beckhoff_ccat_reset_hold(Object *obj, ResetType type)
{
...
static void beckhoff_ccat_class_init(ObjectClass *klass, const void *data)
{
    ...
    rc->phases.hold = beckhoff_ccat_reset_hold;
