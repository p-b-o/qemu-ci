...
static void rp2040_xip_program(RP2040XipState *s)
{
    ...
    rp2040_xip_changed(s, addr, data_len);
    rp2040_xip_writeback_or_warn(s);
    rp2040_xip_set_busy(s, true);
}
...
static void rp2040_xip_erase(RP2040XipState *s)
{
    ...
    erase_len = MIN(FLASH_SECTOR_SIZE, s->flash_size - base);
    memset(&s->storage[base], 0xff, erase_len);
    rp2040_xip_changed(s, base, erase_len);
    rp2040_xip_writeback_or_warn(s);
    rp2040_xip_set_busy(s, true);
}
    ...
    default:
        ...
        if (!s->tx_unsupported_logged) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: unimplemented flash command 0x%02x\n",
                          __func__, s->tx[0]);
...
static bool rp2040_xip_ssi_transfer_active(RP2040XipState *s)
{
    return s->ssienr && (s->ser || !s->qspi_cs_high);
}

static void rp2040_xip_ssi_defer_write(RP2040XipState *s, uint8_t value)
{
    if (s->deferred_tx_len < ARRAY_SIZE(s->deferred_tx)) {
        s->deferred_tx[s->deferred_tx_len++] = value;
    }
}

static void rp2040_xip_ssi_drain_deferred(RP2040XipState *s)
{
    unsigned len;
    unsigned i;

    if (!rp2040_xip_ssi_transfer_active(s)) {
        return;
    }

    len = s->deferred_tx_len;
    s->deferred_tx_len = 0;
    for (i = 0; i < len; i++) {
        rp2040_xip_dr_write(s, s->deferred_tx[i]);
    }
}
                                    ...
                                    unsigned size, MemTxAttrs attrs)
{
    ...
    rp2040_xip_changed(s, addr, size);
    return MEMTX_OK;
}
                                  ...
                                  unsigned size)
{
    ...
    switch (offset) {
    case RP2040_XIP_CTRL:
        new_value = rp2040_atomic_update(s->xip_ctrl, value, alias);
                                 ...
                                 uint64_t value, unsigned size)
{
    qemu_log_mask(LOG_GUEST_ERROR,
                  "%s: write to read-only auxiliary stream FIFO\n",
                  __func__);
}
                                 ...
                                 unsigned size)
{
    ...
    if (offset >= RP2040_SSI_DR0 && offset <= RP2040_SSI_DR_END) {
        if (!rp2040_xip_ssi_transfer_active(s)) {
            rp2040_xip_ssi_defer_write(s, value);
            return;
        }
        rp2040_xip_ssi_drain_deferred(s);
    ...
    switch (offset) {
    case RP2040_SSI_CTRLR0:
        s->ctrlr0 = rp2040_atomic_update(s->ctrlr0, value, alias);
        trace_rp2040_xip_ctrlr0(s->ctrlr0);
        break;
    case RP2040_SSI_CTRLR1:
        s->ctrlr1 = rp2040_atomic_update(s->ctrlr1, value, alias);
        trace_rp2040_xip_ctrlr1(s->ctrlr1);
        break;
    case RP2040_SSI_SSIENR:
        new_value = rp2040_atomic_update(s->ssienr, value, alias);
        ...
        break;
    case RP2040_SSI_SER:
        new_value = rp2040_atomic_update(s->ser, value, alias);
        s->ser = new_value & 1;
        ...
        break;
    case RP2040_SSI_BAUDR:
        new_value = rp2040_atomic_update(s->baudr, value, alias);
        s->baudr = new_value & 0xffff;
        break;
    case RP2040_SSI_TXFTLR:
        new_value = rp2040_atomic_update(s->txftlr, value, alias);
        s->txftlr = new_value & 0xff;
        break;
    case RP2040_SSI_RXFTLR:
        new_value = rp2040_atomic_update(s->rxftlr, value, alias);
        s->rxftlr = new_value & 0xff;
        break;
    case RP2040_SSI_IMR:
        new_value = rp2040_atomic_update(s->imr, value, alias);
        s->imr = new_value & 0x3f;
        break;
    case RP2040_SSI_DMACR:
        new_value = rp2040_atomic_update(s->dmacr, value, alias);
        s->dmacr = new_value & (RP2040_SSI_DMACR_TDMAE |
                                RP2040_SSI_DMACR_RDMAE);
        break;
    case RP2040_SSI_DMATDLR:
        s->dmatdlr = rp2040_atomic_update(s->dmatdlr, value, alias) & 0xff;
        break;
    case RP2040_SSI_DMARDLR:
        s->dmardlr = rp2040_atomic_update(s->dmardlr, value, alias) & 0xff;
        break;
    case RP2040_SSI_RX_SAMPLE_DLY:
        s->rx_sample_dly = rp2040_atomic_update(s->rx_sample_dly, value,
                                                  alias) & 0xff;
        break;
    case RP2040_SSI_SPI_CTRLR0:
        s->spi_ctrlr0 = rp2040_atomic_update(s->spi_ctrlr0, value, alias);
                           ...
                           Error **errp)
{
    ...
    if (rp2040_xip_load_elf(s, filename, &local_err)) {
        ...
        rp2040_xip_changed(s, 0, s->flash_size);
                                  ...
                                  uint8_t block_cmd, Error **errp)
{
    ...
    if (block_size && block_size != 64 * KiB) {
        qemu_log_mask(LOG_UNIMP,
                      "%s: unimplemented block size %" PRIu32 "\n",
                      __func__, block_size);
    }
    if (block_cmd != 0x20 && block_cmd != 0xd8) {
        qemu_log_mask(LOG_UNIMP,
                      "%s: unimplemented erase command 0x%02x\n",
                      __func__, block_cmd);
    }

    memset(s->storage + flash_offs, 0xff, count);
    rp2040_xip_changed(s, flash_offs, count);
...
static void rp2040_xip_realize(DeviceState *dev, Error **errp)
{
    ...
    s->xip_writable = true;
    if (!memory_region_init_rom_device(&s->xip, OBJECT(dev),
                                       &rp2040_xip_ops, s, "rp2040.xip",
                                       s->flash_size, errp)) {
        return;
    }
    s->storage = memory_region_get_ram_ptr(&s->xip);
    ...
    memory_region_init_alias(&s->xip_noalloc, OBJECT(dev),
                             "rp2040.xip.noalloc", &s->xip, 0,
                             s->flash_size);
    memory_region_init_alias(&s->xip_nocache, OBJECT(dev),
                             "rp2040.xip.nocache", &s->xip, 0,
                             s->flash_size);
    memory_region_init_alias(&s->xip_nocache_noalloc, OBJECT(dev),
                             "rp2040.xip.nocache-noalloc", &s->xip, 0,
                             s->flash_size);
