...
static void ssi_enable(QTestState *qts)
{
    qtest_writel(qts, XIP_SSI_BASE + SSI_SSIENR, 1);
}

static void ssi_disable(QTestState *qts)
{
    qtest_writel(qts, XIP_SSI_BASE + SSI_SSIENR, 0);
}

static void ssi_select(QTestState *qts)
{
    qtest_writel(qts, XIP_SSI_BASE + SSI_SER, 1);
}

static void ssi_deselect(QTestState *qts)
{
    qtest_writel(qts, XIP_SSI_BASE + SSI_SER, 0);
}

static void write_enable(QTestState *qts)
{
    ssi_select(qts);
...
static void finish_busy(QTestState *qts)
{
    ssi_select(qts);
...
static void test_ssi_defers_unselected_data(void)
{
    QTestState *qts = rp2040_start(NULL);

    qtest_writel(qts, XIP_SSI_BASE + SSI_DR0, 0x05);
    g_assert_cmphex(qtest_readl(qts, XIP_SSI_BASE + SSI_RXFLR), ==, 0);

    ssi_enable(qts);
    g_assert_cmphex(qtest_readl(qts, XIP_SSI_BASE + SSI_RXFLR), ==, 0);
    ssi_select(qts);
    g_assert_cmphex(qtest_readl(qts, XIP_SSI_BASE + SSI_RXFLR), ==, 1);
    qtest_readl(qts, XIP_SSI_BASE + SSI_DR0);
    ssi_deselect(qts);
...
int main(int argc, char **argv)
{
    ...
    qtest_add_func("/rp2040-xip/ssi-defers-unselected-data",
                   test_ssi_defers_unselected_data);
